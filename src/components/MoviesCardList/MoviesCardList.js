    ...
    return (
        <>
            <div className="grid">
                <ul className="grid__template">
                    {renderMovies.map((movie) => (
                        <MoviesCard savedMovies={savedMovies} key={movie.id} movie={movie}
                                    onBookmarkClick={onBookmarkClick} isSavedMovie={isSavedMovie} />
                        ))
                    }
                </ul>
            </div>
            {currentCount < movies.length &&
                <div className="more">
                    <button className="more__button" type="button" onClick={handleMoreCards}>Ещё</button>
                </div>
            }
        </>
    );
