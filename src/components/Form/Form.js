...
function Form({ formName, submitText, children, submitHandle, validation }) {
  ...
  return (
    <form className="form" action="post" name={`${formName}-form`} onSubmit={submitHandle}>
      ...
      <button type="submit" className="form__button" disabled={!isValid}>{submitText.buttonText}</button>
